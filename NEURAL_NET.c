...
int net_init2 () { //init works! All layers, from 0 with size 73 to 7 with size 3, fully initialized and connected! 
  layers = 8;
  npl = 73;
  int count = 73;
  for (int i = 0; i < layers; i++) {
    for (int j = 0; j < npl; j++) {
      ...
      neur->out[1] = 0;
      nnarray[i][j] = *neur; // insertion of neur into the network array
    }
    if (count > npl) {
  ...
  */
  ...
  for (int i = 1; i < layers; i++) { // through the layers, starting with layer 'two' (1) - IN-CONNECTIONS
	...
	if (nnarray[i][j].used == 1) {
	  nnarray[i][j].in[count1] = nnarray[i - 1][k].layer;
	  nnarray[i][j].in[count2] = nnarray[i - 1][k].pos;
	  count1 = count1 + 2;
	  count2 = count2 + 2;
        }
      }
  ...
  for (int i = 0; i < layers - 1; i++) { // OUT-CONNECTIONS
	...
	if (nnarray[i][j].used == 1) {
	  nnarray[i][j].out[count1] = nnarray[i + 1][k].layer;
	  nnarray[i][j].out[count2] = nnarray[i + 1][k].pos;
	  count1 = count1 + 2;
	  count2 = count2 + 2;
	}
      }
  ...
  for (int i = 0; i < layers; i++) {
	...
	printf(" IN-OUT\n");
        for (int k = 0; k < npl + 10; k++) {
	  if (nnarray[i][j].used == 1 && ((nnarray[nnarray[i][j].out[count1]][nnarray[i][j].out[count2]].layer) != 0 || (nnarray[nnarray[i][j].out[count1]][nnarray[i][j].out[count2]].pos) != 0)) { 
	    printf("%d %d; ", nnarray[nnarray[i][j].in[count1]][nnarray[i][j].in[count2]].layer, nnarray[nnarray[i][j].in[count1]][nnarray[i][j].in[count2]].pos);
	    printf("%d %d| ", nnarray[nnarray[i][j].out[count1]][nnarray[i][j].out[count2]].layer, nnarray[nnarray[i][j].out[count1]][nnarray[i][j].out[count2]].pos);
	  }
	  else {
...
int fillfront () { // loads up the starter neurons
  for (int i = 0; i < 73; i++) {
    double convert;
  ...
  for (int i = 0; i < 73; i++) {
    printf("%f\n", nnarray[0][i].weight); // DEBUG STMT
    nnarray[0][i].value = convarr[i];
    nnarray[0][i].weighted_val = nnarray[0][i].value * nnarray[0][i].weight;
    printf("%f (weighted val), %f (val), %f (weight), %f bias, indice %d\n", nnarray[0][i].weighted_val, nnarray[0][i].value, nnarray[0][i].weight, nnarray[0][i].bias, i); // DEBUG STMT
  }
}
...
int fire () {
  for (int i = 0; i < layers - 1; i++) {
    for (int j = 0; j < npl; j++) {
      for (int k = 0; k < npl - 10; k++) {
	double check;
	check = (nnarray[i + 1][k].weighted_val + nnarray[i][j].weighted_val) * nnarray[i + 1][k].weight * nnarray[i + 1][k].bias;
	//printf("Currently at nnarray[%d][%d], have just evalled one at [%d][%d]...", i, j, i + 1, k); // DEBUG STMT
	if (check >= nnarray[i + 1][k].threshold) {
	  nnarray[i + 1][k].weighted_val = sigmoid2(check);
...
double exemplifyfxn () { // very simple - "heightens" contrast between nodes (small nodes smaller, large nodes larger)
  ...
  */
  ...
  npl = 73;
  decomp(nnarray[7][0].weighted_val, nnarray[7][1].weighted_val, nnarray[7][2].weighted_val); // pass into decomp, the actual backprop fxn
  fire();
  npl = 73;
  return tbr;
}
